...
class CRC32(object):
  ...
  def scan_and_check(self, buff, packet_size):
      current_position = 0
      while current_position+packet_size<buff.length():
          check_block = buff[current_position:packet_size+current_position]
          if self.calc(check_block) == buff[packet_size:packet_size+4]: #if the crc matches
              return check_block[10:packet_size]
          else:
              current_position+=1
