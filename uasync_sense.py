...
class Comm:
    ...
    def __init__(self):
        ...
        print('ID is: ', self.ID)
        self.address_book = {'Server': b'\x00\x13\xA2\x00\x40\xC3\x3F\x72',
                              91: b'\x00\x13\xA2\x00\x40\xDA\x73\x6B',
                              92: b'\x00\x13\xA2\x00\x40\xDA\x73\x67',
                              93: b'\x00\x13\xA2\x00\x40\xDA\x73\x70',
                              94: b'\x00\x13\xA2\x00\x40\xDA\x73\x71'}
    def id_counter(self):
        if self.zbee_id<255:
            self.zbee_id+=1
        else:
...
class ControlTasks:
    ...
    def send_and_wait(self, byte_chunk, addr):
        ...
        success = b'\x00'
        if status == success:
            print('success sending: ', frame_id)
            return
        else:
            #wait a second first
