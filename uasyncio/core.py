...
class EventLoop:
    ...
    def __init__(self):
        self.q = []
        self.cnt = 0
        self.timer_increment = 0
        
    def time(self):
        return time.time()+self.timer_increment
        #return pyb.millis()+self.timer_increment
    
    def increment_time(self, increment):
        self.timer_increment += increment
    ...
    def run_forever(self):
        while True:
            if self.q:
                ...
                #if __debug__:
                #print("Next coroutine to run: %s", (t, cnt, cb, args))
#                __main__.mem_info()
                tnow = self.time()
                delay = t - tnow
                if delay > 0:
