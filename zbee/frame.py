...
class APIFrame:
    ...
    def output(self):
        ...
        #print('len bytes is: ', self.len_bytes())
        data = self.len_bytes() + self.data + self.checksum()
        #print('in output data is: ', data)
    ...
    def remaining_bytes(self):
        ...
        if len(self.raw_data) >= 3:
            ...
            data_len = struct.unpack("> h", raw_len)[1] # for some weird reason this is the opposite of regular python3
            #print('in remaining, data_len is: ', data_len)
            remaining += data_len
    ...
    def parse(self):
        ...
        """
        if len(self.raw_data) < 3:
            print("parse() may only be called on a frame containing at least 3 bytes of raw data (see fill())")
        ...
        # Unpack it
        data_len = struct.unpack("> h", raw_len)[1] #again somehow micropython needs to use [1] instead of [0]
        
        # Read the data
        data = self.raw_data[3:3 + data_len]
        chksum = self.raw_data[-1]
        #print('data is: ', self.raw_data)
        # Checksum check
        self.data = data
